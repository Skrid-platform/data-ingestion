...
class Measure:
    ...
    def __init__(self, source: str, id_: str, events: list[list[Event]] = [], repeat_sign: str | None = None, left: str | None = None, right: str | None = None):
        '''
        Initate Measure.
    ...
    def to_cypher(self, parent_cypher_id: str, previous_Measures: list[Self] = []) -> str:
        '''
        ...
        for voice_index, events_of_voice in enumerate(self.events):
            for k, e in enumerate(events_of_voice):
                if k == 0: # This is the first event of the measure
                    # Try to get the last event (which can be in the last measure, or further than that)
                    i = -1 # Previous measure index
                    while (
                        -i <= len(previous_Measures)
                        and (
                            len(previous_Measures[i].events) <= voice_index
                            or len(previous_Measures[i].events[voice_index]) == 0
                        )
                    ):
                        i -= 1
